...
{
    // 1. ScriptableObject 참조 필드 추가
    [Header("Dialogue Data")]
    [Tooltip("대사 설정이 포함된 ScriptableObject 파일")]
    [SerializeField] private EnemyDialogueData dialogueDataAsset;

    [Header("Dialogue UI Settings")]
    [Tooltip("대사 버블 전체를 감싸는 GameObject (표시/숨김용)")]
    public GameObject DialogueBubble;

    [Tooltip("대사 텍스트를 출력할 TextMeshProUGUI 컴포넌트")]
    public TMPro.TextMeshProUGUI DialogueText;

    [Tooltip("대사 표시 지속 시간")]
    public float DialogueDuration = 3.0f;

    // private EnemyDialogue _dialogueData; // <-- 제거
    ...
    public void DisplayDialogue(string text)
    {
        ...
        {
            // UI가 연결되지 않았다면 기존처럼 로그만 출력
            Debug.LogWarning("Dialogue UI 컴포넌트가 Enemy Inspector에 연결되지 않았습니다! 로그로만 출력됩니다: " + text);
            Debug.Log($"[{id} {gameObject.name}] : \"{text}\"");
            return;
        }

        // 이전 코루틴이 있다면 중지하고 새로운 대사 출력 코루틴 시작
    ...
    private IEnumerator ShowDialogueCoroutine(string dialogue)
    {
        ...
        // TextMeshProUGUI에 텍스트 설정
        DialogueText.text = dialogue;
        DialogueBubble.SetActive(true);

        // DialogueDuration 시간만큼 대기
        yield return new WaitForSeconds(DialogueDuration);

        // 대사가 끝나면 버블을 비활성화하고 코루틴 참조 제거
        DialogueBubble.SetActive(false);
        _currentDialogueCoroutine = null;
    }

    // 2. 랜덤 대사 추출 함수 추가 (기존 GetDialogueData() 대체)
    // 현재 상태에 맞는 대사 목록에서 랜덤으로 하나를 선택해 반환
    // <param name="state">대사를 가져올 FSM 상태 (EnemyState Enum 사용)</param>
    // 랜덤으로 선택된 대사 문자열
    public string GetRandomDialogue(EnemyState state)
    {
        if (dialogueDataAsset == null)
        {
            Debug.LogError("EnemyDialogueData Asset이 Enemy Inspector에 연결되지 않았습니다! 대사 출력 불가.");
            return string.Empty;
        }

        // ScriptableObject에서 ID와 상태에 맞는 대사 리스트를 가져옴
        // ID를 사용해 EnemyDialogueData.cs에서 1002001, 1002002 등 특정 데이터를 찾을 수 있음
        List<string> dialogueList = dialogueDataAsset.GetDialogueList(id, state);

        if (dialogueList == null || dialogueList.Count == 0)
        {
            return string.Empty; // 대사가 없으면 빈 문자열 반환
        }
